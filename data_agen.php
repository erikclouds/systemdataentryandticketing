          ...
          <table class="table table-striped table-advance table-hover" >
              ...
              <tr>
                ...
                <select class="pilihan" name="nama_agen">
                  <option value="ALL">ALL Agen</option>
  ...
  <br>
          ...
          <tbody>
              <tr>
                <th>No</th>
                <th>Nama Agen</th>
                <th>Jumlah Penumpang Tetap</th>
                <th>Jumlah Penumpang Batal</th>
...
if(isset($_POST['cari'])){
  ...
  if ($_POST['nama_agen'] == "ALL") {
       $jml_agen = mysqli_query($conn, "SELECT agen_pendaftar FROM data_penumpang WHERE tanggal_berangkat BETWEEN '".$tanggal_awal."' and '".$tanggal_akhir."'" );      
  }else{
       $jml_agen = mysqli_query($conn, "SELECT agen_pendaftar FROM data_penumpang WHERE tanggal_berangkat BETWEEN '".$tanggal_awal."' and '".$tanggal_akhir."' and agen_pendaftar = '".$nama_agen."'" );      
  }
  $jml_semua_agen = 0;
  while ($d = mysqli_fetch_array($jml_agen)) {
    $nama_agen2[] = $d['agen_pendaftar'];
    $jml_semua_agen++;
  }
  $nama_agen_pertama = $nama_agen2[0];
  $var_agen[0] = $nama_agen2[0];
for ($i=0; $i < $jml_semua_agen; $i++) { 
  if ($nama_agen_pertama != $nama_agen2[$i]) {
        $var_agen[] = $nama_agen2[$i];
        $nama_agen_pertama = $nama_agen2[$i];
  }else{
  ...
  $n1=0;
  $n2=0;
  $nm_agn = array_unique($nama_agen2);
  $nn = count($nm_agn);

for ($i=0; $i < $nn; $i++) { 

  $agen_aktiv = mysqli_query($conn, "SELECT * FROM data_penumpang WHERE tanggal_berangkat BETWEEN '".$tanggal_awal."' and '".$tanggal_akhir."' and status_reservasi = 'ACTIVE' and agen_pendaftar = '".$var_agen[$i]."'" );
  $agen_batal = mysqli_query($conn, "SELECT * FROM data_penumpang WHERE tanggal_berangkat BETWEEN '".$tanggal_awal."' and '".$tanggal_akhir."' and status_reservasi = 'BATAL' and agen_pendaftar = '".$var_agen[$i]."'" );
  while ($dt = mysqli_fetch_array($agen_aktiv)) {
    $n1++;
  }
  ...
  $no++;
    ...
    <tr>
      ...
      <td>
        <?php
          if ($prosentase <= 0.025 && $prosentase >= 0.01){
            $nilai_prosentase = "Baik";
            }elseif($prosentase >= 0.026 && $prosentase <= 0.05){
              $nilai_prosentase = "Buruk";
            }elseif($prosentase >= 0.051 && $prosentase <= 1){
              $nilai_prosentase = "Sangat Buruk";
            }elseif($n2 == 0){
              $nilai_prosentase = "Bagus";
            }
